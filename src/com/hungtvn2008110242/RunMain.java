...
public class RunMain {
    ...
    private static void timKiemTheoKhoangNgay(LocalDate tuNgay, LocalDate denNgay) {
        ...
        for (int i = 0; i < hangHoas.size(); i++) {
            if (hangHoas.get(i).getNgayNhapKho().isAfter(tuNgay)
                    && hangHoas.get(i).getNgayNhapKho().isBefore(denNgay)) {
    ...
    // nếu hàng tồn tại thì trả về vị trí của hàng đó trong danh sách, nếu không tìm
    // thấy trả về -1
    ...
    private static void themDuLieu() {
        hangHoas.add(
                new HangHoa(Loai.THUCPHAM, "THUCPHAM1", "KẸO", 5000, 100, LocalDate.parse("10/11/2019", formatter)));
        hangHoas.add(
                new HangHoa(Loai.THUCPHAM, "THUCPHAM2", "BÁNH", 3000, 10, LocalDate.parse("13/09/2017", formatter)));
        hangHoas.add(new HangHoa(Loai.THUCPHAM, "THUCPHAM3", "TRỨNG", 200000, 120,
                LocalDate.parse("14/08/2018", formatter)));
        hangHoas.add(new HangHoa(Loai.THUCPHAM, "THUCPHAM4", "THỊT LỢN", 17000, 110,
                LocalDate.parse("21/05/2019", formatter)));
        hangHoas.add(
                new HangHoa(Loai.THUCPHAM, "THUCPHAM5", "THỊT GÀ", 4000, 40, LocalDate.parse("11/03/2020", formatter)));
        hangHoas.add(
                new HangHoa(Loai.THUCPHAM, "THUCPHAM6", "CÀ RỐT", 5000, 80, LocalDate.parse("28/01/2021", formatter)));

        hangHoas.add(
                new HangHoa(Loai.SANHSU, "SANHSU1", "BÁT SỨ CON", 15000, 30, LocalDate.parse("11/12/2019", formatter)));
        hangHoas.add(
                new HangHoa(Loai.SANHSU, "SANHSU2", "BÁT SỨ TO", 33000, 101, LocalDate.parse("21/04/2017", formatter)));
        hangHoas.add(
                new HangHoa(Loai.SANHSU, "SANHSU3", "CHÉN SỨ", 21000, 21, LocalDate.parse("19/01/2018", formatter)));
        hangHoas.add(
                new HangHoa(Loai.SANHSU, "SANHSU4", "ẤM TRÀ", 290000, 10, LocalDate.parse("22/02/2019", formatter)));
        hangHoas.add(new HangHoa(Loai.SANHSU, "SANHSU5", "NỒI NINH SỨ", 230000, 20,
                LocalDate.parse("13/07/2020", formatter)));
        hangHoas.add(
                new HangHoa(Loai.SANHSU, "SANHSU6", "VÒ RƯỢU", 500000, 60, LocalDate.parse("15/01/2021", formatter)));

        hangHoas.add(new HangHoa(Loai.DIENMAY, "DIENMAY1", "MÁY GIẶT", 6200000, 100,
                LocalDate.parse("11/11/2019", formatter)));
        hangHoas.add(new HangHoa(Loai.DIENMAY, "DIENMAY2", "MÁY LỌC NƯỚC", 3000000, 10,
                LocalDate.parse("11/09/2017", formatter)));
        hangHoas.add(new HangHoa(Loai.DIENMAY, "DIENMAY3", "NỒI CƠM ĐIỆN", 1200000, 120,
                LocalDate.parse("11/08/2018", formatter)));
        hangHoas.add(new HangHoa(Loai.DIENMAY, "DIENMAY4", "BẾP ĐIỆN", 1700000, 110,
                LocalDate.parse("11/05/2019", formatter)));
        hangHoas.add(new HangHoa(Loai.DIENMAY, "DIENMAY5", "MÁY SẤY TÓC", 100000, 40,
                LocalDate.parse("11/03/2020", formatter)));
        hangHoas.add(new HangHoa(Loai.DIENMAY, "DIENMAY7", "MÁY BƠM", 2000000, 80,
                LocalDate.parse("11/01/2021", formatter)));
        hangHoas.add(new HangHoa(Loai.DIENMAY, "DIENMAY8", "MÁY HÚT BỤI", 1500000, 80,
                LocalDate.parse("11/01/2021", formatter)));
        hangHoas.add(new HangHoa(Loai.DIENMAY, "DIENMAY9", "TV MÀN HÌNH CON", 5000000, 80,
                LocalDate.parse("11/01/2021", formatter)));
    }
}
